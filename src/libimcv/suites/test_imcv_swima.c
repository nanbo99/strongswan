...
START_TEST(test_imcv_swima_ev)
{
	...
	attr = ietf_swima_attr_sw_ev_create(sw_ev_data[_i].flags,
										...
										sw_ev_data[_i].last_consulted_eid,
										sw_id_only);
	...
	{
		c_attr->set_last_eid(c_attr, sw_ev_data[_i].last_eid);
	}
	...
	eid = c_attr->get_last_eid(c_attr, NULL, NULL);
	ck_assert(eid == sw_ev_data[_i].last_consulted_eid);
	eid = c_attr->get_last_eid(c_attr, &epoch, &last_eid);
	ck_assert(eid == sw_ev_data[_i].last_consulted_eid);
