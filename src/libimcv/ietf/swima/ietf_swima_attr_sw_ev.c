	...
	private_ietf_swima_attr_sw_ev_t *this)
{
	...
	writer->write_uint32(writer, this->last_consulted_eid);
...
{
	bio_reader_t *reader;
	uint32_t data_model_pen, record_id, eid;
	...
	if (this->offset == 0)
	{
		...
		reader->read_uint32(reader, &this->last_consulted_eid);
...
METHOD(ietf_swima_attr_sw_ev_t, set_last_eid, void,
	private_ietf_swima_attr_sw_ev_t *this, uint32_t last_eid)
{
	this->last_eid = last_eid;
}

METHOD(ietf_swima_attr_sw_ev_t, get_last_eid, uint32_t,
	private_ietf_swima_attr_sw_ev_t *this, uint32_t *eid_epoch,
	uint32_t *last_eid)
{
	if (eid_epoch)
	{
...
{
	this->events->destroy(this->events);
	this->events = swima_events_create();
}
