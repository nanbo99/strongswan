 ...
 */
static bool add_sw_inventory(imc_state_t *state, imc_msg_t *msg,
							 ...
							 swima_inventory_t *targets)
{
	...
	inventory = collector->collect(collector, sw_id_only, targets);
	items = inventory->get_count(inventory);
	DBG1(DBG_IMC, "collected %d SWID tag%s%s", items , sw_id_only ? " ID" : "",
											   items == 1 ? "" : "s");

	/* Send an IETF SW [Identity] Inventory attribute */
	attr = ietf_swima_attr_sw_inv_create(IETF_SWIMA_ATTR_SW_INV_FLAG_NONE,
									request_id, eid_epoch, 1, sw_id_only);
	sw_inv_attr = (ietf_swima_attr_sw_inv_t*)attr;
	sw_inv_attr->set_inventory(sw_inv_attr, inventory);
...
static TNC_Result receive_message(imc_state_t *state, imc_msg_t *in_msg)
{
	...
	while (enumerator->enumerate(enumerator, &attr))
	{
		...
		if (!add_sw_inventory(state, out_msg, request_id, sw_id_only, targets))
		{
			break;
		}
	}
